...
const MOCK_INCIDENTS: MapPoint[] = [
  {
    ...
    description:
      "Kejadian begal terjadi sekitar dini hari pukul 02:00 WIB. Pelaku berboncengan menggunakan senjata tajam.",
    latitude: -6.273,
    longitude: 106.815,
  ...
  },
  {
    ...
    description:
      "Tawuran antar kelompok remaja menggunakan batu dan petasan. Kepolisian telah membubarkan massa.",
    latitude: -6.209,
    longitude: 106.849,
  ...
  },
  {
    ...
    description:
      "Helm fullface hilang diambil orang di area parkiran luar motor. Kejadian siang hari.",
    latitude: -6.139,
    longitude: 106.883,
  ...
  },
  {
    ...
    description:
      "Korban mengalami luka di tangan setelah dihadang geng motor saat pulang kerja larut malam.",
    latitude: -6.155,
    longitude: 106.755,
  ...
  },
  {
    id: "mock-5",
    sourceType: "USER_REPORT",
    title: "Begal Sadis di Margonda Depok",
    description:
      "Korban diancam celurit dan motor matic berhasil dibawa kabur pelaku berjumlah 3 orang.",
    latitude: -6.372,
    longitude: 106.832,
...
export default function MapComponent() {
  ...
  const [activeTab, setActiveTab] = useState<"peta" | "lapor" | "chat">("peta");
  ...
  >([
    {
      sender: "ai",
      text: "Halo! Saya SafeRoute AI. Tanyakan kepada saya mengenai kerawanan kriminalitas, rute teraman, atau tips berkendara aman.",
  ...
  const createCustomIcon = (type: string, riskLevel: string) => {
    ...
    if (riskLevel === "CRITICAL")
      color = "bg-rose-600 border-2 border-white shadow-rose-600/30";
    else if (riskLevel === "HIGH")
      color = "bg-orange-500 border-2 border-white shadow-orange-500/30";
    else if (riskLevel === "MEDIUM")
      color = "bg-amber-500 border-2 border-white shadow-amber-500/30";
    else if (riskLevel === "LOW")
      color = "bg-emerald-500 border-2 border-white shadow-emerald-500/30";

    return L.divIcon({
      html: `<div class="relative flex h-8 w-8 items-center justify-center rounded-full text-[10px] font-bold text-white shadow-lg ${color} transition-all duration-300 hover:scale-110">
        <span>${iconLabel}</span>
      </div>`,
      className: "",
      iconSize: [32, 32],
  ...
  const startIcon = L.divIcon({
    html: `<div class="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-500 border-4 border-white shadow-xl animate-pulse">
  ...
  const destIcon = L.divIcon({
    html: `<div class="flex flex-col items-center justify-center h-10 w-8">
      <div class="flex h-8 w-8 items-center justify-center rounded-full bg-slate-900 border-2 border-white shadow-xl text-white">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="h-4 w-4 text-emerald-400"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>
      </div>
    </div>`,
    className: "",
    iconSize: [32, 40],
  ...
  useEffect(() => {
    ...
    const fetchRoute = async () => {
      ...
      try {
        const res = await fetch(
          `https://router.project-osrm.org/route/v1/driving/${startPoint.lng},${startPoint.lat};${destPoint.lng},${destPoint.lat}?overview=full&geometries=geojson`
        );
        const data = await res.json();
    ...
    fetchRoute();
  }, [startPoint, destPoint, incidents]);
  ...
  const handleSendChat = () => {
    ...
    setChatMessages((prev) => [
      ...prev,
      { sender: "user", text: userMsg, time: timeString },
    ]);
    setChatInput("");
    setAiTyping(true);
    ...
    setTimeout(() => {
      ...
      if (textLower.includes("depok") || textLower.includes("margonda")) {
        const count = incidents.filter(
          (i) =>
            i.address?.toLowerCase().includes("depok") ||
            i.title.toLowerCase().includes("depok")
        ).length;
        reply = `Kondisi di Margonda Depok terdeteksi rawan kriminalitas. Tercatat ada ${count || 1
          } insiden begal yang terkonfirmasi baru-baru ini. Kami menyarankan untuk tidak berkendara sendirian lewat tengah malam, gunakan jalan protokol yang terang benderang.`;
      } else if (textLower.includes("kemang") || textLower.includes("selatan")) {
        reply =
          "Kawasan Kemang, Mampang Prapatan terdeteksi memiliki 1 laporan aktif mengenai begal motor di malam hari. Tetap melaju di rute ramai dan hindari gang sempit tak berlampu.";
      } else if (
        textLower.includes("sunter") ||
        textLower.includes("mall") ||
        textLower.includes("utara")
      ) {
        reply =
          "Sunter Mall terpantau kondusif dari tindak kekerasan jalanan, namun waspadai pencurian kendaraan bermotor (curanmor) di area parkir luar yang minim pengawasan.";
      } else if (
        textLower.includes("begal") ||
        textLower.includes("kriminal") ||
        textLower.includes("rawan")
      ) {
        reply =
          "Untuk menghindari aksi begal: \n1. Selalu berkendara berkelompok di malam hari.\n2. Rencanakan rute Anda menggunakan SafeRoute (gunakan tab Peta).\n3. Berhenti hanya di tempat ramai atau SPBU jika merasa diikuti.";
      } else if (textLower.includes("cara") && textLower.includes("lapor")) {
        reply =
          "Untuk melaporkan kejadian, Anda cukup klik 'Lapor' di menu navigasi, pilih jenis insiden, tentukan lokasi di peta, isi deskripsi kronologi lengkap, lalu klik Kirim. Laporan Anda akan ditinjau admin dalam beberapa menit.";
      } else {
        reply =
          "SafeRoute AI siap memandu perjalanan Anda. Jika Anda ingin memeriksa rute teraman, Anda bisa memasukkan destinasi Anda di tab Peta. Apakah ada wilayah tertentu yang ingin Anda tanyakan?";
      }
    ...
    <div className="flex h-screen w-screen flex-col overflow-hidden bg-slate-50 font-sans">
      <header className="z-[1000] flex h-16 w-full shrink-0 items-center justify-between border-b border-slate-100 bg-white px-6 shadow-sm">
        ...
        <div className="flex h-full items-center gap-8">
          <Link
            href="/home"
            className={`relative flex h-full items-center px-1 text-sm font-semibold transition-all duration-200 ${true ? "text-[#0B2540]" : "text-slate-500 hover:text-slate-800"
              }`}
          >
            Peta
          ...
          <Link
            href="/lapor"
            className={`relative flex h-full items-center px-1 text-sm font-semibold transition-all duration-200 ${false ? "text-[#0B2540]" : "text-slate-500 hover:text-slate-800"
              }`}
          >
            Lapor
          </Link>
            ...
            {showUserDropdown && (
              <div className="absolute right-0 mt-2 w-52 rounded-2xl border border-slate-100 bg-white p-2.5 shadow-xl ring-1 ring-black/5 z-[2000] animate-fade-in">
                {user ? (
                  ...
                  <div className="p-1">
                    <p className="px-2.5 py-1.5 text-[11px] text-slate-500 font-medium leading-relaxed">
                      Masuk untuk melaporkan insiden kriminalitas di sekitar Anda.
                    </p>
                    <Link
                      href="/login"
                      onClick={() => setShowUserDropdown(false)}
                      className="mt-2 block w-full rounded-xl bg-blue-600 py-2 text-center text-xs font-bold text-white hover:bg-blue-700 transition-colors shadow-md shadow-blue-500/10"
                    >
                      Masuk / Daftar
                    </Link>
                  </div>
              ...
              <Marker position={[startPoint.lat, startPoint.lng]} icon={startIcon}>
                <Popup>
                  <div className="p-1 text-xs">
                    ...
                    <p className="text-slate-500 mt-0.5">{startPoint.name}</p>
                  </div>
                </Popup>
              </Marker>
            )}
              ...
              <Marker position={[destPoint.lat, destPoint.lng]} icon={destIcon}>
                <Popup>
                  <div className="p-1 text-xs">
                    ...
                    <p className="text-slate-500 mt-0.5">{destPoint.name}</p>
                  </div>
                </Popup>
              </Marker>
            )}
                ...
                <Popup className="custom-popup">
                  <div className="p-2.5 max-w-xs text-xs font-sans">
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`rounded px-1.5 py-0.5 text-[9px] font-bold text-white ${inc.riskLevel === "CRITICAL"
                          ? "bg-rose-600"
                          : inc.riskLevel === "HIGH"
                            ? "bg-orange-500"
                      ...
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                        {inc.sourceType === "ML_CRAWLER"
                          ? "ML CRAWLER"
                          : "LAPORAN WARGA"}
                      </span>
                    </div>

                    <h4 className="mt-2 font-bold text-slate-800 text-sm leading-tight">
                      {inc.title}
                    </h4>
            ...
            {showTileSelector && (
              <div className="absolute right-12 top-0 w-36 rounded-2xl border border-slate-100 bg-white p-2 shadow-xl animate-fade-in z-20">
                ...
                <button
                  ...
                  className="w-full text-left rounded-xl px-2.5 py-1.5 text-xs font-semibold hover:bg-slate-100 transition-colors mt-1 text-slate-700"
                >
                  Standard Map
                </button>
                <button
                  ...
                  className="w-full text-left rounded-xl px-2.5 py-1.5 text-xs font-semibold hover:bg-slate-100 transition-colors text-slate-700"
                >
                  Sleek Voyager
                </button>
                <button
        ...
        {clickMode !== "none" && (
          <div className="absolute left-1/2 top-6 z-10 -translate-x-1/2 rounded-full bg-slate-900/90 border border-slate-800 px-6 py-2.5 text-xs font-semibold text-white shadow-xl backdrop-blur-sm animate-bounce flex items-center gap-2">
            ...
            <button
              ...
              className="ml-2 rounded-full bg-white/20 hover:bg-white/30 px-2 py-0.5 text-[10px]"
            >
              Batal
            </button>
          </div>
        ...
        <div className="absolute left-6 top-6 bottom-6 z-10 w-80 max-w-sm shrink-0 flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white/95 backdrop-blur-md shadow-2xl transition-all duration-300">
          <div className="flex flex-1 flex-col overflow-y-auto p-5">
            ...
            <p className="mt-1 text-[11px] text-slate-400 leading-normal font-medium">
              Pilih titik awal dan tujuan Anda untuk menganalisis tingkat keamanan rute
              perjalanan.
            </p>

            <div className="relative mt-5">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">
                Titik Awal (Lokasi Anda)
              </label>
              <div className="flex gap-1.5 items-center">
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full rounded-xl bg-slate-50 border border-slate-200/60 py-2.5 pl-3 pr-8 text-xs font-semibold text-slate-700 outline-none focus:bg-white focus:border-blue-500 transition-colors"
                  />
                  {startInput && (
                    <button
                      onClick={() => {
                ...
                <button
                  ...
                  className={`rounded-xl border p-2.5 shadow-sm transition-all duration-200 ${clickMode === "start"
            ...
            <div className="relative mt-4">
              ...
              <div className="flex gap-1.5 items-center">
                ...
                <button
                  ...
                  className={`rounded-xl border p-2.5 shadow-sm transition-all duration-200 ${clickMode === "dest"
                    ? "bg-blue-600 border-blue-600 text-white"
                    : "bg-white border-slate-200 text-slate-500 hover:bg-slate-50"
                    }`}
                  title="Pilih di Peta"
            ...
            {routeLoading ? (
              <div className="mt-8 flex flex-col items-center justify-center p-6 text-center border border-dashed border-slate-200 rounded-2xl">
                ...
                <p className="mt-1 text-[10px] text-slate-400">
                  Menghubungi OSRM & memetakan koordinat...
                </p>
              </div>
            ) : routePolyline.length > 0 ? (
              <div className="mt-6 flex-1 flex flex-col gap-4 animate-fade-in">
                {/* Route Summary Card */}
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest flex items-center gap-1">
                        <Shield className="h-3.5 w-3.5 fill-emerald-400 text-emerald-400" />
                        Rute Teraman
                      </p>
                      <h3 className="mt-1 text-2xl font-black">
                        {routeDistance ? (routeDistance / 1000).toFixed(1) : 0}{" "}
                    ...
                    <div className="text-right">
                      ...
                      <p className="mt-0.5 text-sm font-bold flex items-center justify-end gap-1 text-slate-200">
                        <Clock className="h-3.5 w-3.5" />
                        {routeDuration ? Math.round(routeDuration / 60) : 0} mnt
                      </p>
                    </div>
                  </div>

                  <div className="mt-3.5 border-t border-slate-800 pt-3 flex items-center justify-between text-xs">
                    <span className="text-slate-400 font-medium">Tingkat Bahaya:</span>
                    <span
                      className={`rounded-full px-2.5 py-0.5 text-[10px] font-extrabold ${incidentsNearRoute.length > 2
                  ...
                  {incidentsNearRoute.length > 0 ? (
                    <div className="mt-3 max-h-40 overflow-y-auto flex flex-col gap-2 pr-1">
                      {incidentsNearRoute.map((inc) => (
                        <div
                          key={inc.id}
                          className="rounded-xl border border-slate-100 bg-white p-2.5 flex flex-col gap-1 shadow-sm"
                        >
                          <div className="flex justify-between items-center">
                            <span className="text-[10px] font-extrabold text-slate-800 truncate max-w-[140px]">
                              {inc.title}
                            </span>
                            <span
                              className={`rounded px-1 text-[8px] font-extrabold text-white ${inc.riskLevel === "CRITICAL"
                                ? "bg-rose-600"
                                : inc.riskLevel === "HIGH"
                                  ? "bg-orange-500"
                                  : inc.riskLevel === "MEDIUM"
                    ...
                    <p className="mt-4 text-xs font-semibold text-slate-500 text-center flex flex-col items-center justify-center py-2">
                      <span>🎉 Tidak ada titik kejahatan terdeteksi di sepanjang jalur ini!</span>
                      <span className="text-[10px] text-slate-400 font-medium mt-1">
                        Sistem menyarankan rute ini aman dilalui.
                      </span>
                    </p>
                  )}
                </div>
              ...
              <div className="mt-8 flex flex-col items-center justify-center p-6 text-center border border-dashed border-slate-200 rounded-2xl">
                ...
                <p className="mt-2.5 text-xs font-bold text-slate-500">
                  Rute Belum Terbentuk
                </p>
                <p className="mt-1 text-[10px] text-slate-400">
                  Masukkan titik awal dan tujuan Anda untuk menampilkan jalur teraman di
                  peta.
                </p>
                <button
                  onClick={handleLocateUser}
                  className="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 px-4 py-2 text-xs font-bold text-blue-600 transition-colors shadow-sm"
                >
